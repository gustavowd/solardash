...
setup_page('Inversores', 'Explore a produção e as variáveis de operação dos inversores do campus.')
...
df_m = conn.query("select * from measurement_type order by measurement_type_id limit 21")
...
if len(d) == 2:
    ...
    for m in multi:
        ...
        df = conn.query("select measurement_value, measurement_time from measurements where measurement_time between '" + str(d[0]) + " 05:00:01' and '" + str(d[1]) + " 19:59:59' and device_id=" + str(device_id) + " and measurement_type_id=" + str(measure_id) + " order by measurement_time", ttl=0)
        if not(df.empty):
            if chart_data.empty:
