...
df = conn.query("select * from devices where device_type=5 order by device_name")
devices = pd.DataFrame(df, columns=['device_name', 'device_id', 'device_type'])
devices = devices.set_index('device_id')

df_m = conn.query("select measurement_name, measurement_type_id from measurements natural join measurement_type where device_id=41 and measurement_time between '2025-04-06 20:00:01' and '2025-04-06 20:00:15' order by measurement_type_id")
